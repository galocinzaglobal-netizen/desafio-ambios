...
export function LeadScreen({ onRestart, onRanking }: { onRestart: () => void; onRanking: (name: string) => void }) {
  const [sent, setSent] = useState(false);
  const submit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const name = new FormData(event.currentTarget).get("name")?.toString() ?? ""; setSent(true); onRanking(name); };
